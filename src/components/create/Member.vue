<template>
  <section id="memberInfo" class="verticalFlex">
        <h2>멤버</h2>
        <div>
            <select name="" id="memberLevel" v-model="unionStorage.member.level">
                <option value="">계급 선택</option>
                <option v-for="(value, key) in memberList" :key="key" v-bind:value="key">
                    {{key}}
                </option>
            </select>
            <input id="memberName" type="text" v-model="unionStorage.member.name">
        ...
        <div class="memberWrap horizontalFlex">
            <div>
                ...
                <div class="horizontalFlex" v-if="memberList.leader">
                    ...
                    <button class="del" @click="unionStorage.deleteMember('leader', 0)">취소</button>
                </div>
            </div>
            <div>
                <h4>부대장</h4>
                <ul>
                    <li v-for="(member, index) in memberList.commander" :key="index" class="horizontalFlex">
                        <span>{{member}}</span>
                        <button class="del" @click="unionStorage.deleteMember('commander', index)">취소</button>
                    </li>
                </ul>
            </div>
            <div>
